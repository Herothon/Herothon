...
async def startupmessage():
    try:
        if BOTLOG:
            Config.CATUBLOGO = await iqthon.tgbot.send_file(BOTLOG_CHATID, "https://telegra.ph/file/5247b3ed40021926946b9.jpg", caption="♛ ⦙ تـمّ  اعـادة تشـغيل\n  هيـرو ثـون ✓  :  [ 7.6 ] .\n\n♛ ⦙ للحصول على اوامر السورس\n أرسـل : (  `.اوامري`  ) \n\n♛ ⦙ لمـعرفة كيفية تغير بعض كلايش\n او صور السـورس  أرسـل  :\n (  `.مساعده`  )\n\n♛ ⦙ القناة الرسمية هيرثون : @iQkoot",                buttons=[(Button.url("مطور هيروثون الرسمي", "https://t.me/TToooT"),)],            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(            "تخزين هيرثون العام", iqthon, Config.TG_BOT_USERNAME, descript  ,  iqphoto1 )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        _, groupid = await create_supergroup(            "تخزين هيرثون الخاص", iqthon, Config.TG_BOT_USERNAME, descript    , iqphoto2  )
